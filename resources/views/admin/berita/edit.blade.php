                    ...
                    <div class="row g-3">
                            ...
                            <div class="mb-3">
                                ...
                                @if ($berita->gambar)
                                    <div class="mt-3">
                                        ...
                                        <img src="{{ asset('storage/' . $berita->gambar) }}" alt="Cover Berita" class="img-thumbnail" style="max-height: 120px;" />
                                    </div>
                                @endif
                            </div>
                        </div>
                                    ...
                                    @forelse($tags as $tag)
                                        <div class="col-md-3 col-6 mb-2">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="tags[]" value="{{ $tag->id }}" id="tag_{{ $tag->id }}" {{ in_array($tag->id, old('tags', $beritaTagIds)) ? 'checked' : '' }}>
                                                <label class="form-check-label text-truncate" for="tag_{{ $tag->id }}">{{ $tag->nama }}</label>
                                            </div>
                                        </div>
                                    @empty
                                        <div class="col-12">
                                            <span class="text-muted small">Belum ada tag yang dibuat.</span>
                                        </div>
                                    @endforelse
                                </div>
                            </div>
                        ...
                        <div class="col-12 border-top pt-3">
                            <div class="mb-3 form-check">
                                <input type="checkbox" class="form-check-input" id="is_published" name="is_published" value="1" {{ old('is_published', $berita->is_published) ? 'checked' : '' }}>
                                <label class="form-check-label fw-bold text-dark" for="is_published">Publikasikan Berita</label>
